...
var Provider = {
    ...
    getAll: function(name) {
        var result = [],
            suffixResult = [],
            filter = name ? filterPrefix(name) : null,
            suffixFilter = name ? filterSuffix(name) : null;
        for(var serviceName in this.$module) {
            if (serviceName.indexOf(servicePropertyPrefix) > -1) {
        ...
        if(result.length) {
            ...
            if (suffixResult.length === 1) {
                return mapServices(suffixResult, this)[0];
            } else {
